...
In terms of biological applications, the paper discusses how FMRP in a mRNA binding protein that interacts with Dgkk to mediate synaptic plasticity. While the rest of the paper focuses on the implications of this interactions, they do not discuss the other mRNA species that the microarray shows show similar changes in Log Fold-Change and p-value. In my analysis, I too found that these same species were also significantly higher suggesting that these proteins might be interacting with multiple RNA species or could be part of an RNA binding complex. One other interesting point is that there are multiple isoforms of FMRP which might interact with a completely non-redundant RNA species. I feel that their interpretation that Dgkk is the silver bullet in terms of FMRP interactors is not a sufficiently answer to the story, and as such the other highly significant interactors should be investigated more fully. 

# Conclusions on Robustness and Reproducability 
...
I believe that this is something numerous people discussed in their presentations, suggesting that this problem is not just an oversite by a few people, but by many studies using bioinformatic approaches. The lack of detail within these methods results in people not being able to adequately reanalyze and reinterpret data. While we spent a good amount of class talking about how robustness and reproducibility, this can also have a negative effect on other inferences based on that data. For example, in the dataset I analyzed, there were other mRNA species that had highly significant hits, this could potentially hold within it projects for many other labs to begin investigating. The inability to reanalyze data due to missing details, whether purposeful or not wastes precious funding that could be put to better use. For these reasons, and those discussed in class, I believe that reproducibility and robustness should be held to higher standards when manuscripts are submitted for publication. 
